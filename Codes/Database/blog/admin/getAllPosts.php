<?php
	...
	if(isset($_POST['user_name'])){
		...
		$userID = $db->query("SELECT * FROM sub WHERE title = '$name'");
		...
		$query = $db->prepare("SELECT post_id, user_id, title, body,category_id, posted, uVote, dVote, EditDate, subsaidit, subID FROM posts WHERE subID = '$var'");
		$query->execute();
	...
	<body>
		...
		<div id = "get-userName">
			<form action = "<?php echo $_SERVER['PHP_SELF']?>" method = "post">
				<div>
						<label>SubSaiddit Title: </label><input type = "text" name ="user_name" />
				</div>
				<input type = "submit" name = "submit" value = "Find All Posts"/>
			</form>
		</div>
	</body>
</html>